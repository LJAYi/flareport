...
For this MVP, PR creation never enables auto-merge. `auto` and eligible `staged-auto` policies mean “create the PR automatically”; their decision remains `pending-checks`. `GitHubClient.enableAutoMerge` is reserved for a later signed `check_run`/deployment handler that verifies the exact PR head SHA and configured required checks. This is intentionally conservative: branch protection alone is not treated as proof that trusted checks were configured.
